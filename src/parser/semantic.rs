...
use crate::parser::error::{ArgumentMismatch, CheckResult, IncompatibleTypes, ParseError, VoidOperand, WrongOperandType, WrongReturnType};
use crate::parser::sym_table::{ResultType, Type};
...
pub fn check(node: &Node) -> CheckResult {
    ...
    match node.node_type() {
        SyntaxElement::Class => {}
        SyntaxElement::Init => {}
        SyntaxElement::Enter => { check_return(node)?; }
...
pub fn check_return(node: &Node) -> CheckResult {
    let proc_entry = node.get_obj().as_ref().expect("enter has no sym table entry").borrow();
    ...
    /// a `while` (in which case you treat it as a sub-sequence and check whether IT returns the ret_type),
    ...
    fn check_execution_path(start_node: &Node, ret_type: Type) -> bool {
        ...
        while let Some(current_node) = c_node {
            match current_node.node_type() {
                SyntaxElement::Assign | SyntaxElement::Var | SyntaxElement::Const | SyntaxElement::Binop(_) | SyntaxElement::Call => {
